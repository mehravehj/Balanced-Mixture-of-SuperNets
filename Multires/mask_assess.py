...
from model_1 import multires_model
...
def main():
    ...
    print(net.layer[0].block.block.bn1[0].momentum)
    print(net)
    ...
    best_model, epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index = load_checkpoint(save_dir, net)
    aa = net.state_dict()
    # print(aa['layer.0.block.alpha'])
    # print(aa['layer.0.block.block.conv1.weight'][0])
    # print(aa['layer.5.block.block.conv1.weight'][1])
    ...
    for layer in range(0,10):
        ...
        for counter in range(0, 5):
            l = layer
            net.layer[l].block.remove = counter
            print('epoch ', epoch)
            ...
            if test_accuracy < accuracy_layer:
                accuracy_layer = test_accuracy
                res_to_select = counter
            ...
            print('layer',layer, 'resolution', counter)
            ...
            # net.layer[l].block.remove = 0
        res_per_layer.append(res_to_select)
        print('res for layer', l , ' selected ', res_to_select)
        net.layer[l].block.remove = res_to_select
...
def load_checkpoint(save_dir, model):#, weight_optimizer, scheduler, alpha_optimizer):
    ...
    if path.exists(save_dir):
        ...
        # print(checkpoint['best_model']['layer.0.block.alpha'])
        # print(checkpoint['best_model']['layer.0.block.block.conv1.weight'][0])
        # print(checkpoint['best_model']['layer.1.block.block.conv1.weight'][1])
        ...
        # print(aa['layer.0.block.alpha'])
        # print(aa['layer.0.block.block.conv1.weight'][0])
        # print(aa['layer.1.block.block.conv1.weight'][1])
        # print('......................................')
...
def train_valid(train_queue, validation_queue, model, criterion=nn.CrossEntropyLoss(), epoch=0):
    model.train()
    train_loss = 0
    validation_loss = 0
    train_correct = 0
