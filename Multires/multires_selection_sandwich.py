...
from model_selection_sandwich import multires_model
...
def main():
    ...
    finetune_epoch = 6 # epochs to train after filter selection

    print('Test ', args.test_name)
    print(args)

    #load network
    save_dir = './checkpoint/ckpt_' + str(args.test_name) + '.t7' #selection test number
    if path.exists(save_dir): # if already selected from trained model, continue loading
        ...
        net = multires_model(ncat=ncat, net_type=args.net_type, mscale=args.mscale, channels=args.channels,
                             leng=args.leng, max_scales=args.max_scales, factor=args.factor,
                             initial_alpha=args.initial_alpha, pool=selected_resolutions, current_layer=layer)

    else:
        selected_resolutions = {0:1, args.leng-1:args.max_scales}
        layer = 0
        net = multires_model(ncat=ncat, net_type=args.net_type, mscale=args.mscale, channels=args.channels,
                             leng=args.leng, max_scales=args.max_scales, factor=args.factor,
                             initial_alpha=args.initial_alpha, pool=selected_resolutions, current_layer=layer)
    ...
    best_model, current_epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index, layer, selected_resolution = load_checkpoint_selection(save_dir, net, weight_optimizer, scheduler,
                                                                  alpha_optimizer)
    ...
    for epoch in range(current_epoch+1, args.epochs+1):
        ...
        if epoch % finetune_epoch == 0:
            ...
            selected_resolutions = select_max(selected_resolutions, best_alpha, layer, args.leng)
            # selected_resolutions = select_max(selected_resolutions, get_alpha(args.net_type, net))
            print('resolutions ', selected_resolutions[layer], 'and', selected_resolutions[args.leng-layer-1], ' selected....')
            # resolution_change_epoch['epoch'].append(epoch)
            # resolution_change_epoch['resolution_selection'].append(selected_resolutions)
            best_alpha = 0
...
def select_max(selected_resolutions, alpha, layer, lenght):
    res_up = int(np.argmax(alpha[0])) + 1
    res_bottom = int(np.argmax(alpha[-1])) + 1
    selected_resolutions[layer] = res_up
    selected_resolutions[lenght-layer-1] = res_bottom
...
def load_checkpoint_new_layer(save_dir, model, weight_optimizer, scheduler, alpha_optimizer):
    print('Loading from checkpoint...')
    checkpoint = torch.load(save_dir)
    model_dict = model.state_dict()
    weight_dict = {k: v for k,v in model_dict if not ('alpha' in k)}
...
def save_checkpoint_selection(save_dir, model, best_model, weight_optimizer, scheduler, alpha_optimizer_state, epoch,
                              loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index, current_layer, selected_res):
    state = {
