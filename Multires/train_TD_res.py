...
parser.add_argument('--path_res', '-pt', type=int, default=0, help='which path to take')
...
def main():
    ...
    for epoch in range(epoch+1, args.epochs+1):
        ...
        res_loss, res_prob, best_paths, worst_paths = \
            train_function(train_loader, validation_loader, net, weight_optimizer, criterion, loss_per_res_vector)
        # scheduler.step()
...
def train_valid(train_queue, validation_queue, model, weight_optimizer, criterion, loss_per_res):
    '''
    ...
    paths = {}
    path = [args.path_res for _ in range(args.leng)]
    path[0] = 0
    path = torch.tensor(path)
    model.set_path(path)
    for batch_idx, (train_inputs, train_targets) in enumerate(train_queue):
        # sample and set path
        # avg_loss, prob = res_distribution(loss_per_res)
        #train model
        model.train()
    ...
    avg_loss_end = 0
    prob_end = 0
    # print('probabilities', res_distribution(loss_per_res))
...
def res_distribution(loss_per_res):
    # print(loss_per_res)
    avg = torch.mean(loss_per_res, 2)
    # print(avg)
    # print(avg.size())
    prob = nn.functional.softmax(avg.data*args.factor, dim=1)
    # print(prob.size())
    # print('prob', prob)
