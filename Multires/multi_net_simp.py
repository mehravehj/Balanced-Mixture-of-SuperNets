...
from func_multi_simp import sResBlock
...
class multi(nn.Module):
    def __init__(self, ncat=10, mscale='sconv', channels=32, leng=10, max_scales=4, usf=True, factor=1, lf=False,
        ...
        if self.dense:
            ...
            if self.lfb:
                ...
                self.bfactor = 1
        if alinit:
            init_alpha = init_alpha.split(',')
            init_alpha = [int(i) for i in init_alpha]
            # init_alpha = [i in init_alpha for i in range(leng)]
            print(init_alpha)
        ...
        if mscale == 'normal':
            ...
            if 1:
                listc = [
                    sConv2d(3, channels, 3, max_scales, usf, factor=1, alinit=alinit, athr=0, lf=0, prun=0, alpha=self.init_alpha[0])]
                listc += [sConv2d(channels, channels, 3, max_scales, usf, factor=1, alinit=alinit, athr=0, lf=0, prun=0,
                                  alpha=self.init_alpha[i + 1]) for i in range(leng - 2)]
                listc += [sConv2d(channels, ncat, 3, max_scales, usf, factor=1, alinit=alinit, athr=0, lf=0, prun=0,
                                  alpha=self.init_alpha[-1])]
        elif mscale == 'sres':
            if 1:
                # listc = [nn.Conv2d(3, channels, kernel_size=3, stride=1, padding=1)]
                listc = [sResBlock(3, channels, 3, max_scales, usf, factor=1, alinit=0, athr=0, lf=0, prun=0,
                                    alpha=self.init_alpha[0])]
                # listc =  sConv2d(3, channels, 3, max_scales, usf, factor=1, alinit=0, athr=0, lf=0, prun=0,
                #             alpha=self.init_alpha[0])]
                listc += [sResBlock(channels, channels, 3, max_scales, usf, factor=1, alinit=0, athr=0, lf=0, prun=0,
                                    alpha=self.init_alpha[i + 1]) for i in range(leng-2)]
                # listc += [sConv2d(channels, ncat, 3, max_scales, usf, factor=1, alinit=0, athr=0, lf=0, prun=0,
                #                   alpha=self.init_alpha[-1])]
                # listc += [ nn.Conv2d(channels, ncat, kernel_size=3, stride=1, padding=1)]
                # self.relu1 = nn.ReLU()
                listc += [sResBlock(channels, ncat, 3, max_scales, usf, factor=1, alinit=0, athr=0, lf=0, prun=0,
                                  alpha=self.init_alpha[-1])]
        self.conv = nn.ModuleList(listc)

        # self.conv1 = nn.ModuleList(listc)
        # self.fc = nn.Linear(channels, ncat)
    ...
    def forward(self, x):
        ...
        if self.mscale == 'sres':
            ...
            # out = x
            # out = self.conv[0](out)
            # for c in range(len(self.conv)-2):
            #     out = self.conv[c+1](out)
            #     out = self.relu1(out)
            # out = self.conv[-1](out)
...
net = multi(ncat=ncat, mscale=args.mscale, channels=args.channels, leng=args.leng, max_scales=args.max_scales,
            ...
            lf=0, alinit=args.alinit, init_alpha=args.init_alpha, athr=0, prun=0,
            dense=0,
...
if args.resume:
    ...
    if not args.save_name:
        ...
        if scheduler:
            ...
            #         optimizernet, factor= 0.5, patience=15)
...
def train_valid(epoch):
    ...
    #     print('net learning rate: ', optimizernet.param_groups[0]['lr'])
    net.train()
    loss_train = 0
    loss_valid = 0
    train_correct = 0
    ...
    for batch_idx, (train_inputs, train_targets) in enumerate(trainloader):
        ...
        t_loss.backward(retain_graph=True)#TODO: activate MiLeNAS
        # t_loss.backward()
        optimizernet.step()
        ...
        if epoch > args.a_train_start - 1:
            ...
            val_loss = criterion(val_outputs, val_targets) #TODO: change to val_loss for MiLeNAS
            '''
                MiLeNAS: add training loss to validation loss
                training loss calculated again?
            '''
            # t_loss = criterion(train_outputs, train_targets)
            v_loss = val_loss + t_loss    #training loss added to validation loss   #TODO: MiLeNAS
            v_loss.backward()
            optimizeralpha.step()
    ...
    if scheduler:
        scheduler.step()  # TODO: ReduceLROnPlateau
    # print(val_targets)
        ...
        for l in net.modules():
            ...
            if args.dense:
                ...
                if args.lfb:
                    bfactor = net.module.bfactor.data.cpu().numpy()
                    bfactors.append(bfactor)
    # print(nalphas, factors, nbetas, bfactors)
