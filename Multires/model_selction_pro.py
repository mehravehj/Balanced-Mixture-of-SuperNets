...
def select_max(alpha, current_layer):
    alpha[current_layer] = int(torch.argmax(alpha[current_layer])) + 1
    return alpha


class multires_model(nn.Module):
    def __init__(self, ncat=10, net_type='multires', mscale='sconv', channels='32', leng=3, max_scales=2, factor=1, initial_alpha='0', pool='0', current_layer=0):
        ...
        self.initial_alpha = select_max(self.initial_alpha, current_layer)
