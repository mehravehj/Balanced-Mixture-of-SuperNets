...
class path_model(nn.Module):
    ...
    def __init__(self, ncat=10, net_type='multires', mscale='sconv', channels='32', leng=3, max_scales=2):
        ...
        self.channels = list_layer_wise(channels, leng-1) # channels per layer, 1st and last layers are 3 and ncat

        list_layer = [network_layer( mscale, 3, self.channels[0], 3)]

        list_layer += [network_layer(mscale, self.channels[i-1], self.channels[i], 3) for i in range(1, leng - 1)]

        list_layer += [network_layer(mscale, self.channels[-1], ncat, 3)]
        # self.bn = nn.BatchNorm2d(ncat, affine=False)
        self.layer = nn.ModuleList(list_layer)

    def set_path(self, path):
        self.path = [i.item() for i in path]
        self.relative_path = relative_path(self.path)
        # print(self.path)
        self.sample_functions = [sampling_func(i) for i in self.relative_path]
        # print('sample function', self.sample_functions)

    def forward(self, x):
        out = x
        for l in range(self.leng):

            # print('relative path', self.relative_path[l])
            out = self.sample_functions[l](out, self.relative_path[l])
