...
def data_loader(dataset, valid_percent, batch_size, indices):
    ...
    if dataset == 'CIFAR10':
        ...
        # testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=test_transform_CIFAR)
        num_class = 10
    elif dataset == 'STL10': # TODO: server  data directory modification
        trainset = torchvision.datasets.STL10(
            root='~/Desktop/codes/multires/data/', split='train', download=True,
        ...
        testset = torchvision.datasets.STL10(
            root='~/Desktop/codes/multires/data/', split='test', download=True,
    ...
    if valid_percent:  # create validation set
        num_train = len(trainset)# size of all available training set
        train_size = num_train - int(valid_percent * num_train)# number of traning examples
        ...
        if dataset == 'TIN':
            ...
            if not indices: # if indices is not passed to the function
                indices = list(range(num_train)) # available indices at training set
                shuffle(indices)
                if 1:   # TODO: overfit test, remove when running test
                    num_train = 5000  # size of all available training set
                ...
                val_indc = indices[split:]  #TODO: deactivate for MiLeNAS
                # val_indc = indices    #TODO: activate for MiLeNAS
                # suffle(val_ind)
                indices = [train_indc, val_indc]
            else:
                train_indc = indices[0]
        ...
        trainloader = torch.utils.data.DataLoader(
            ...
            sampler=torch.utils.data.sampler.SubsetRandomSampler(train_indc), num_workers=0, pin_memory=True)#trainign set
        valloader = torch.utils.data.DataLoader(
            valset, batch_size=batch_size,
            sampler=torch.utils.data.sampler.SubsetRandomSampler(val_indc),  num_workers=0, pin_memory=True)#validation ste
...
class sConv2d(nn.Module):
    def __init__(self, in_, out_, kernel_size, max_scales=4, usf=True, factor=1, alinit=0, athr=0, lf=0, prun=0,
        ...
        if alinit:
            print('costum initial alpha')
            self.alpha = nn.Parameter(torch.eye(self.max_scales)[alpha])
        else:
            print('uniform  initial alpha')
    ...
    def forward(self, x):
        ...
        ly = []
        for r in self.corr:#if athr is 0, idx = r
...
class sResBlock(nn.Module):
    def __init__(self, in_, out_, kernel_size, max_scales=4, usf=True, factor=1, alinit=0, athr=0, lf=0, prun=0,
        ...
        if self.usf:
            ...
            self.relu = nn.ReLU()
            self.bn1 = nn.ModuleList([nn.BatchNorm2d(out_, affine=False) for _ in self.corr])
            self.conv2 = nn.Conv2d(out_, out_, kernel_size, stride=1, padding=1)
            self.bn2 = nn.ModuleList([nn.BatchNorm2d(out_, affine=False) for _ in self.corr])
            if in_ != out_:
                self.conv3 = nn.Conv2d(in_, out_, kernel_size=1, stride=1, padding=0, bias=False)
                self.bn3 = nn.ModuleList([nn.BatchNorm2d(out_, affine=False) for _ in self.corr])
    ...
    def forward(self, x):
        ...
        for r in self.corr:#if athr is 0, idx = r
            ...
            self.nalpha[r] = sel[idx]
            if self.usf:
                # print('lx size ', lx[0].size())
                if self.in_ != self.out_: # shortcut
                    resid = self.conv3(lx[r])
                    resid = self.bn3[idx](resid)
                    residual.append(resid)
                else:
                    residual.append(lx[r])
                # print('lx size ', lx[0].size())
                y = self.conv1(lx[r])
                y = self.bn1[idx](y)
                y = self.relu(y)
