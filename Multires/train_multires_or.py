...
def main():
    ...
    if args.dataset == 'CIFAR10':
        ...
        ncat = 200
    '''create model'''
    net = multires_model(ncat=ncat, net_type=args.net_type, mscale=args.mscale, channels=args.channels, leng=args.leng, max_scales=args.max_scales, factor=args.factor, initial_alpha=args.initial_alpha, pool=args.pooling)
    net.cuda()
    ...
    # print(macs, params)
    # print('----------------------------')
    # macs, params = get_model_complexity_info(net, (3, 64, 64), as_strings=False,
    ...
    #separate optimizer for weight ans alpha parameters
    weight_parameters, alpha_parameters = parameters(net) #get conv and alpha optimizers
    weight_optimizer = optim.SGD(weight_parameters, lr=args.learning_rate, momentum=args.weight_momentum,
                                 weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(weight_optimizer, float(args.epochs),
    ...
    '''loading model (resuming)'''
    save_dir = './checkpoint/ckpt_' + str(args.test_name) + '.t7'
    best_model, epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index \
    ...
    '''execution (train, validate, test and save'''
    for epoch in range(epoch+1, args.epochs+1):
        ...
        if epoch % 2 == 0 or epoch == 1:
            ...
            #record progress: loss, accuracy, alpha
            ...
            print('.....SAVING.....')
            weight_optimizer_state = weight_optimizer.state_dict()
            alpha_optimizer_state = alpha_optimizer.state_dict()
            #saving checkpoint
            save_checkpoint(save_dir, net, best_model, weight_optimizer_state, scheduler, alpha_optimizer_state, epoch, loss_progress,
...
def train_valid(train_queue, validation_queue, model, weight_optimizer, alpha_optimizer, criterion=nn.CrossEntropyLoss(), epoch=0):
    ...
    for batch_idx, (train_inputs, train_targets) in enumerate(train_queue):
        ...
        #     # for l in range(args.leng):
        #     #     print('alpha update ratio')
        #     #     print('layer ', l)
        #     #     print(model.layer[l].block.alpha.grad * args.alr)
        #
        #
...
def train(train_queue, validation_queue, model, weight_optimizer, alpha_optimizer, criterion=nn.CrossEntropyLoss(), epoch=0):
    ...
    for batch_idx, (train_inputs, train_targets) in enumerate(train_queue):
        ...
        # train_loss += train_minibatch_loss.cpu().item()
        train_loss.append(train_minibatch_loss.cpu().item())
        train_correct, train_total = calculate_accuracy(train_outputs, train_targets, train_total, train_correct)
    # for l in range(args.leng):
    #     print('conv update ratio')
    #     print('layer ', l)
