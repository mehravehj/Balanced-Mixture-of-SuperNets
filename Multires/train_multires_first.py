...
parser.add_argument('--warm_restart', '-mr', type=int, default=0, help='number of workers to load data')
parser.add_argument('--first', '-fr', default=False, action='store_true', help='first resume that uses warm restarts')
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(weight_optimizer, float(150),
                                                           eta_min=args.min_learning_rate)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(weight_optimizer, T_0=225, T_mult=1.5,eta_min=args.min_learning_rate)
    ...
    best_model, epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index, scheduler = load_checkpoint(save_dir, net, weight_optimizer, scheduler, alpha_optimizer)
    if args.first:
        weight_optimizer = optim.SGD(weight_parameters, lr=args.learning_rate*0.5, momentum=args.weight_momentum,
                                     weight_decay=args.weight_decay)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(weight_optimizer, T_0=150, T_mult=1,
                                                                        eta_min=args.min_learning_rate)
    else:
        if epoch%150:
            weight_optimizer = optim.SGD(weight_parameters, lr=args.learning_rate,
                                         momentum=args.weight_momentum,
                                         weight_decay=args.weight_decay)
            best_model, epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index, scheduler = load_checkpoint(
                save_dir, net, weight_optimizer, scheduler, alpha_optimizer)
        else:
            lr_factor = (1 / 2)**int(int(epoch)/150)
            weight_optimizer = optim.SGD(weight_parameters, lr=args.learning_rate*lr_factor, momentum=args.weight_momentum,
                                     weight_decay=args.weight_decay)
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(weight_optimizer, T_0=150, T_mult=1,
                                                                             eta_min=args.min_learning_rate)
        # best_model, epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index, scheduler = load_checkpoint(save_dir, net, weight_optimizer, scheduler, alpha_optimizer)
...
def load_checkpoint(save_dir, model, weight_optimizer, scheduler, alpha_optimizer):
    ...
    return best_model, epoch, loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index, scheduler
