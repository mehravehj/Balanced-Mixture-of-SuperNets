...
from model_prog import multires_model
...
parser.add_argument('--layer_epoch', '-le', type=int, default=100, help='epochs to train')
...
parser.add_argument('--mscale', '-ms', type=str, default='sres', help='sconv or sres')
...
def main():
    ...
    print(args)
    ncat = 10
    net = multires_model(ncat=ncat, mscale=args.mscale, channels=args.channels, leng=args.leng,
                         max_scales=args.max_scales, usf=args.usf)
    net.cuda()
    ...
    weight_optimizer = optim.SGD(weight_parameters, lr=args.learning_rate, momentum=args.weight_momentum,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(weight_optimizer, args.layer_epoch, eta_min=args.min_learning_rate)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(weight_optimizer, T_0=args.layer_epoch, T_mult=1,
    ...
    alpha_optimizer = optim.SGD(alpha_parameters, lr=args.alr, weight_decay=args.awd)
    ...
    ###loading data
    train_loader, validation_loader, test_loader, indices, num_class = data_loader(args.dataset,
                                                                                   args.validation_percent,
                                                                                   args.batchsize,
                                                                                   num_train=args.number_train,
                                                                                   indices=index,
    ...
    for epoch in range(epoch + 1, args.epochs + 1):
        ...
        train_loss, validation_loss, train_accuracy, validation_accuracy = train_valid(train_loader, validation_loader,
                                                                                       net, weight_optimizer,
                                                                                       alpha_optimizer, criterion,
                                                                                       epoch)
        scheduler.step()
        ...
        if epoch % 2 == 0 or epoch == 0:
            ...
            save_checkpoint(save_dir, net, best_model, weight_optimizer, scheduler, alpha_optimizer_state, epoch,
                            loss_progress,
                            accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index)
            # print(best_alpha)
        if epoch % args.layer_epoch == 0:
            print(best_alpha)
            selected_res = int(np.argmax(best_alpha[layer_prog])) + 1
            res_selected.append(selected_res)
            layer_prog += 1
            print('layer ', layer_prog, 'resolution', selected_res)
            '''reinitialize  alphas only'''
            # create a state dict with only updated alphas
            # current_model = {}
            # for key in net.state_dict():
            #     # print(net.state_dict()[key])
            #     if 'alpha' in key:
            #         current_model[key] = net.state_dict()[key].clone().detach()
            #         print(current_model[key].size(0))
            #         current_model[key].data = torch.ones(current_model[key].size(0))
            # Load only updated alphas
            # net.load_state_dict(current_model, strict=False)
            ...
            weight_optimizer = optim.SGD(weight_parameters, lr=args.learning_rate, momentum=args.weight_momentum,
                                         weight_decay=args.weight_decay)
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(weight_optimizer, args.layer_epoch,
                                                                   eta_min=args.min_learning_rate)
            # alpha_optimizer = optim.Adam(alpha_parameters, lr=args.alr, weight_decay=args.awd)
            alpha_optimizer = optim.SGD(alpha_parameters, lr=args.alr, weight_decay=args.awd)
            for l in range(layer_prog):
                net.conv[l].set_max(res_selected[l])
            print('updated alpha')
            print(get_alpha(net))
            best_accuracy = 0
            best_alpha = 0

            # for i in range(layer_prog, args.leng):
            #     net.conv[i].reinitialize_alpha()
...
def save_checkpoint(save_dir, model, best_model, weight_optimizer, scheduler, alpha_optimizer_state, epoch,
                    loss_progress, accuracy_progress, alpha_progress, best_alpha, best_epoch, best_accuracy, index):
    state = {
...
# def get_alpha( model):
#     alpha = np.zeros((args.leng, args.max_scales+1))
#     k = 0
#     for name, param in model.named_parameters():
#         if param.requires_grad and 'alpha' in name:
#             alpha[k,:param.size(0)] = param.detach().cpu()
#             k += 1
#     return alpha
...
def train_valid(train_queue, validation_queue, model, weight_optimizer, alpha_optimizer,
                criterion=nn.CrossEntropyLoss(), epoch=0):
    model.train()
    train_loss = 0
    validation_loss = 0
    ...
    for batch_idx, (train_inputs, train_targets) in enumerate(train_queue):
        ...
        if epoch >= args.alpha_train_start:
            ...
            validation_correct, validation_total = calculate_accuracy(validation_outputs, validation_targets,
