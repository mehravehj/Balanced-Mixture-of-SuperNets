...
from model_norm import multires_model
...
def main():
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(weight_optimizer, mode='min', factor=0.5, patience=50, cooldown=20, min_lr=args.min_learning_rate, verbose=True)
    ...
    for epoch in range(epoch+1, args.epochs+1):
        ...
        train_loss, validation_loss, train_accuracy, validation_accuracy = train(train_loader, validation_loader, net, weight_optimizer, alpha_optimizer, criterion, epoch)
        validation_loss, validation_accuracy = test(validation_loader, net)
        # scheduler.step()
