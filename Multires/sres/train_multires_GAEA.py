...
from model_GAEA import multires_model
...
parser.add_argument('--fine_tune_epoch', '-fe', type=int, default=100, help='epochs to train')
...
def main():
    ...
    # alpha_optimizer = optim.SGD(alpha_parameters, lr=args.alr, weight_decay=0)
    alpha_optimizer = 0
    ...
    for epoch in range(epoch+1, args.epochs+1):
        ...
        train_loss, validation_loss, train_accuracy, validation_accuracy = train_valid(train_loader, validation_loader, net, weight_optimizer, alpha_parameters, criterion, epoch)
        scheduler.step()
...
def train_valid(train_queue, validation_queue, model, weight_optimizer, alpha_parameters, criterion=nn.CrossEntropyLoss(), epoch=0):
    model.train()
    train_loss = 0
    validation_loss = 0
    ...
    for batch_idx, (train_inputs, train_targets) in enumerate(train_queue):
        ...
        if epoch >= args.alpha_train_start:
            ...
            for param in alpha_parameters:
                ...
                param.data.mul_(dp)
                print('param', param)
                param.data.mul_(1/(torch.sum(param.data).view(-1)))
                print('normalized param')
                # param.data = (self.alpha / torch.sum(self.alpha)).view(-1, 1)
                print('updated', param)
...
class EG_optimizer(optim.Optimizer):
    def __init__(self, params, lr=1e-3):
        defaults = dict(lr=lr)
        super(EG_optimizer, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(EG_optimizer, self).__setstate__(state)
