...
from utils.search_space_design import init_path_logit, sample_uniform, model_prob_calculator, exp_moving_avg, \
    update_path_logits, sample_path_norm
from utils import *
        ...
        for i in range(num_models):
            ...
            for j in range(num_paths):
                ...
                if i == torch.argmax(prob_cond[j, :]):
                    ...
                    print('evaluate model', i, 'from prob', prob_cond[j, :], 'accuracy:', valid_acc_epoch)
...
def accuracy_per_class(logits, target):
    correct_class = torch.zeros(101)
    total_class = torch.zeros(101)
