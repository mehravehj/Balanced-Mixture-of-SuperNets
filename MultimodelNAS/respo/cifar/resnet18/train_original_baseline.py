...
from create_model_baseline import ResNet18, create_search_space
...
parser.add_argument('--weight_decay', '-wd', default=0.005, type=float, help='weight decay')
...
def main_worker(gpu, args):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200,300], gamma=0.1, last_epoch=- 1, verbose=False)
    ...
    if args.dataset=='CIFAR10':
        training_dataset = datasets.CIFAR10(root=args.data_dir, train=True, download=True,
                                            transform=train_transform)  # Data augmentation is only done on training images
        test_dataset = datasets.CIFAR10(root=args.data_dir, train=False, download=True, transform=test_transform)
    elif args.dataset=='CIFAR100':
        training_dataset = datasets.CIFAR100(root=args.data_dir, train=True, download=True,
                                            transform=train_transform)  # Data augmentation is only done on training images
        test_dataset = datasets.CIFAR100(root=args.data_dir, train=False, download=True, transform=test_transform)

    train_loader = torch.utils.data.DataLoader(training_dataset, batch_size=args.batch_size,
                                                  shuffle=True)
