...
from create_model import ResNet18
from utils.NAS_trainer import create_optimizers, train_valid, validate_all
...
parser.add_argument('--workers', '-wr', type=int, default=6, help='number of workers to load data')
...
parser.add_argument('--ema_decay', '-emd', default=0.95, type=float, help='exponential moving average decay')
...
def main_worker(gpu, args):
    ...
    train_loader, validation_loader = data_loader(gpu, args.fp32, dataset_dir, train_file, val_file, args.batch_size, args.workers, seed_np)
    ...
    for epoch in range(current_epoch, args.epochs + 1):
        ...
        if epoch != 0 and (epoch % 100 == 0 or epoch == args.epochs):  # test and save checkpoint every 10 epochs
            print('Saving models and progress...')
            if epoch != 0 and (epoch % 10 == 0 or epoch == args.epochs):
                init_acc_mat, init_acc_mat_per_class = validate_all(nets, num_models, paths, num_paths, validation_loader, prob_mat)
...
def save_checkpoint(save_dir, models, optimizers, schedulers, epoch,t_acc, v_acc, t_loss, v_loss, c_matrix, w_matrix, prob_mat, p_marginal, num_models, acc_mat, acc_mat_per_class, temp):
    state = {
        't_loss': t_loss,
        't_acc': t_acc,
        'v_loss': v_loss,
